...
export const EmployerEditJobPage = () => {
  const { id } = useParams<{ id: string }>();
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [saving, setSaving] = useState(false);

  const { values, setValues, errors, touched, isSubmitting, formError, handleChange, handleBlur, handleSubmit } = useFormValidation({
    schema: jobSchema,
    initialValues: {
      title: '',
      company: '',
    ...
    const load = async () => {
      ...
      try {
        ...
        setValues({
          ...
          type: (job.type as JobValues['type']) ?? 'HIRING',
          experienceLevel: (job.experienceLevel as JobValues['experienceLevel']) ?? 'ENTRY_LEVEL',
          workplaceType: (job.workplaceType as JobValues['workplaceType']) ?? 'ONSITE',
  ...
  if (loading) {
    return <div className="page fade-in"><div className="loading-state"><span className="spinner" aria-hidden="true" /><span>Loading job…</span></div></div>;
  }
  ...
  return (
    <div className="page fade-in">
      <PageHeader title="Edit job" subtitle="Update your job posting." />
